      ...
      if (useStream && (analysisMode === 'validated' || analysisMode === 'optimized' || analysisMode === 'fast')) {
        ...
        await handleSSEStream(response, {
          onChunk: (content, accumulatedText) => {
            console.log('📡 [MRI] Получен чанк:', content.length, 'символов, всего:', accumulatedText.length)
          ...
          onComplete: (finalText) => {
            console.log('✅ [MRI STREAMING] Streaming завершён успешно, итого:', finalText.length, 'символов')
            flushSync(() => {
              setResult(finalText)
            })
          }
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      ...
      <AnalysisTips 
        content={{
          ...
          optimized: "рекомендуемый режим (Gemini JSON + Sonnet 4.5) — идеальный баланс точности и цены для МРТ‑исследований.",
          ...
          extra: [
            "⭐ Рекомендуемый режим: «Оптимизированный» (Gemini + Sonnet) — идеальный баланс цены и качества для МРТ‑исследований.",
      ...
      <AnalysisResult result={result} loading={loading} mode={mode} imageType="mri" />
