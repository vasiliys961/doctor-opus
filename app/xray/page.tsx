...
import { xrayPageMessages } from '@/lib/i18n/ui-client-messages'
...
export default function XRayPage() {
  ...
  const t = xrayPageMessages[locale]
  ...
  const analyzeImage = async (analysisMode: AnalysisMode, useStream: boolean = true) => {
    ...
    try {
      ...
      if (isComparisonMode && archiveFile) {
        ...
        formData.append('prompt', REQUEST_PROMPTS.xray.comparative)
      } else {
        formData.append('prompt', REQUEST_PROMPTS.xray.single)
      }
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      ...
      <AnalysisTips 
        content={{
          ...
          extra: [
            ...
            "⚠️ **Claude Opus 5**: NOT recommended for this section (weakest model for imaging).",
      ...
      <div className="bg-white rounded-lg shadow-lg p-4 sm:p-6 mb-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-semibold">{t.uploadImages}</h2>
          <button
            onClick={() => {
          ...
          >
            {isComparisonMode ? `✅ ${t.comparisonOn}` : `📊 ${t.comparisonOff}`}
          </button>
        </div>
      ...
      {(imagePreview || archivePreview) && (
        <div className="bg-white rounded-lg shadow-lg p-4 sm:p-6 mb-6">
          <h2 className="text-xl font-semibold mb-4">📷 {isComparisonMode ? t.comparisonView : t.uploadedImage}</h2>
          
          <div className={`grid grid-cols-1 ${isComparisonMode && archivePreview ? 'md:grid-cols-2' : ''} gap-4`}>
            {imagePreview && (
              <div className="flex flex-col items-center">
                {isComparisonMode && <span className="text-xs font-bold text-gray-500 mb-1 uppercase tracking-widest">{t.current}</span>}
                <img 
                  src={imagePreview} 
                  alt={t.current}
            ...
            {isComparisonMode && archivePreview && (
              <div className="flex flex-col items-center">
                <span className="text-xs font-bold text-blue-600 mb-1 uppercase tracking-widest">{t.archiveBefore}</span>
                <img 
                  src={archivePreview} 
                  alt={t.archiveBefore}
