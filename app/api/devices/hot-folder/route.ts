...
const DICOM_EXTENSIONS = new Set([
  '.dcm',
  '.dicom',
  '.ima',
  '.acr',
...
const isAllowedMime = (mime: string): boolean => {
  ...
  return (
    ...
    lower === 'application/x-dicom' ||
    lower === 'application/octet-stream'
  )
}

const isDicomLikeByNameOrMime = (fileName: string, mimeType: string): boolean => {
  const normalizedName = fileName.toLowerCase()
  const lowerMime = mimeType.toLowerCase()
  if (lowerMime.includes('dicom')) return true
  return Array.from(DICOM_EXTENSIONS).some((ext) => normalizedName.endsWith(ext))
}

const looksLikeDicom = (bytes: Buffer): boolean => {
  if (bytes.length >= 132) {
    const magic = bytes.subarray(128, 132).toString('ascii')
  ...
  if (bytes.length >= 8) {
    const vr1 = bytes[4]
    const vr2 = bytes[5]
    const isAsciiUpper = (code: number) => code >= 65 && code <= 90
    if (isAsciiUpper(vr1) && isAsciiUpper(vr2)) {
      return true
    }
  }

  const probe = bytes.subarray(0, Math.min(bytes.length, 4096)).toString('ascii')
  if (probe.includes('1.2.840.10008')) return true
  return false
}
...
export async function POST(request: NextRequest) {
  ...
  for (const file of inputFiles) {
    ...
    const detectedDicom = looksLikeDicom(bytes)
    const fallbackMime = detectedDicom ? 'application/dicom' : 'application/octet-stream'
    const mimeType = file.type || fallbackMime
    const dicomLikeMeta = isDicomLikeByNameOrMime(file.name || '', mimeType)

    if (!isAllowedMime(mimeType)) {
      return NextResponse.json({ success: false, error: `unsupported mime type: ${mimeType}` }, { status: 400 })
    }

    const isDicomFile = detectedDicom || dicomLikeMeta
    if (isDicomFile) {
      payloadType = 'dicom'
    }
