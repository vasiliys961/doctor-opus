...
`;
    ...
    const LIBRARY_CONTEXT_MARKER = '### КОНТЕКСТ ИЗ БИБЛИОТЕКИ:';
    const pubMedQuery = message.includes(LIBRARY_CONTEXT_MARKER)
      ? message.split(LIBRARY_CONTEXT_MARKER)[0].trim()
      : message.trim();
    ...
    if (enableMedicalBrowsing && isAcademicSearch) {
      ...
      try {
        ...
        pubMedStatusInstruction = articles.length > 0
          ? `В начале ответа первой строкой укажи: "🔎 PubMed: найдено ${articles.length} источников."`
          : 'В начале ответа первой строкой укажи: "🔎 PubMed: по текущему запросу источники не найдены."';
      } catch (error) {
        ...
        pubMedStatusInstruction = 'В начале ответа первой строкой укажи: "🔎 PubMed: онлайн-поиск временно недоступен."';
      }
    }
