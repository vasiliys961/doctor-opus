...
import { getDiagnosticsMessages } from '@/lib/i18n/diagnostics';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!session?.user?.email) {
      return NextResponse.json({ success: false, error: i18n.triageAuthRequired }, { status: 401 });
    }
    ...
    if (!rl.allowed) {
      return NextResponse.json(
        { success: false, error: i18n.triageRateLimitExceeded },
    ...
    if (!text.trim()) {
      return NextResponse.json({ success: false, error: i18n.triageEmptyInput }, { status: 400 });
    }

    const result = await classifyChatComplexity(text, locale);
    return NextResponse.json({ success: true, ...result });
  } catch (error: any) {
    return NextResponse.json(
      { success: false, error: error?.message || getDiagnosticsMessages().triageFailed },
      { status: 500 }
    );
  }
}
