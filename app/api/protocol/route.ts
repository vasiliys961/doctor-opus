 ...
 * (соответствует Python бекенду: use_sonnet_4_5=True)
 */
export async function POST(request: NextRequest) {
  try {
    console.log('🚀 [PROTOCOL API] ========== НАЧАЛО ГЕНЕРАЦИИ ПРОТОКОЛА ==========');
    ...
    if (!rawText || !rawText.trim()) {
      console.error('❌ [PROTOCOL API] Ошибка: текст не предоставлен');
      return NextResponse.json(
        { success: false, error: 'Текст для протокола не предоставлен' },
        { status: 400 }
      );
    }
...
Медицинские рекомендации - опираться только на проверенные международные источники; для каждого ключевого лечебного шага указывать ссылку и год публикации (предпочтительно ≤5 лет).`;
    ...
    if (useStreaming) {
      console.log('');
      console.log('🤖 [MODEL INFO] ========== ГЕНЕРАЦИЯ ПРОТОКОЛА (STREAMING) ==========');
      console.log('🤖 [MODEL INFO] Модель:', MODEL);
      console.log('🤖 [MODEL INFO] Max tokens:', MAX_TOKENS);
      console.log('🤖 [MODEL INFO] Размер промпта:', `${prompt.length} символов`);
      console.log('🤖 [MODEL INFO] Режим: streaming');
      console.log('📤 [PROTOCOL API] Отправка streaming запроса к OpenRouter...');
      console.log('');
    ...
    console.log('');
    console.log('🤖 [MODEL INFO] ========== ГЕНЕРАЦИЯ ПРОТОКОЛА (ОБЫЧНЫЙ РЕЖИМ) ==========');
    console.log('🤖 [MODEL INFO] Модель:', MODEL);
    console.log('🤖 [MODEL INFO] Max tokens:', MAX_TOKENS);
    console.log('🤖 [MODEL INFO] Размер промпта:', `${prompt.length} символов`);
    console.log('🤖 [MODEL INFO] Режим: обычный (не streaming)');
    console.log('');
    
    let result = await sendTextRequest(prompt, []);
    
    console.log('');
    console.log('📝 [PROTOCOL API] ========== ОТВЕТ ОТ МОДЕЛИ ПОЛУЧЕН ==========');
    console.log('📝 [PROTOCOL API] Модель:', MODEL);
    console.log('📝 [PROTOCOL API] Длина ответа:', `${result.length} символов`);
    console.log('📝 [PROTOCOL API] Первые 200 символов:', result.substring(0, 200));
    console.log('');

    console.log('✅ [PROTOCOL API] ========== ПРОТОКОЛ УСПЕШНО СГЕНЕРИРОВАН ==========');
    console.log('✅ [PROTOCOL API] Финальный размер:', `${result.length} символов`);
    console.log('');
