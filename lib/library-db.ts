...
function ensureDbSchema(request: IDBOpenDBRequest): void {
...
function openDb(version?: number): Promise<IDBDatabase> {
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = () => ensureDbSchema(request);
  });
}

export async function openLibraryDB(): Promise<IDBDatabase> {
  // Открываем БД без версии, чтобы не получить VersionError, если в браузере
  // уже есть более новая версия (например, после main-ветки/старой сборки).
...
/**
 * Возвращает чанки конкретной страницы документа (для визуального RAG по image match).
 */
export async function getDocumentChunksByPage(
  documentId: string,
  pageId: number,
  limit: number = 2
): Promise<string[]> {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = () => {
      const rows = (request.result as LibraryChunk[]) || [];
      const chunks = rows
        .filter((row) => Number.isFinite(Number(row.pageId)) && Number(row.pageId) === pageId)
        .map((row) => row.content)
        .filter(Boolean)
        .slice(0, Math.max(1, limit));
      resolve(chunks);
...
const MIN_RELEVANCE_SCORE = 0.22;
...
export async function searchLibraryLocalWithMeta(query: string, limit: number = 3): Promise<LibrarySearchHit[]> {
  ...
  const wantsFitzpatrick = /(fitz|fitzpatrick|фитц|фитцпатрик)/i.test(queryLower);
  ...
  const scoredChunks = allChunks.map((chunk, i) => {
    ...
    // Явный запрос книги/атласа по названию: повышаем страницы из соответствующего
    // документа, чтобы модель не "теряла" нужную книгу из-за только текстового скоринга.
    const docName = String(docsById.get(chunk.documentId)?.name || '').toLowerCase();
    const nameHits = expandedKeywords.filter((word) => word.length >= 3 && docName.includes(word)).length;
    if (nameHits > 0) {
      combined += Math.min(0.9, nameHits * 0.25);
    }
    if (wantsFitzpatrick && /(fitz|fitzpatrick|фитц|фитцпатрик)/i.test(docName)) {
      combined += 1.2;
    }

    // Качество источника: чанк без pageId и без валидного документа чаще всего
    // оказывается "служебным мусором" (шаблоны/инструкции), а не реальным
    // фрагментом книги. Сильно понижаем его при ранжировании.
    const hasPageAnchor = Number.isFinite(Number(chunk.pageId));
    const hasDocumentName = Boolean(docsById.get(chunk.documentId)?.name);
    if (!hasPageAnchor) combined *= 0.35;
    if (!hasDocumentName) combined *= 0.5;
  ...
  // Fallback: грубое совпадение по подстроке, но только в "настоящих" чанках
  // с привязкой к странице документа.
  if (results.length === 0) {
    results = allChunks
      .filter((chunk) => {
        const contentLower = chunk.content.toLowerCase();
        return Number.isFinite(Number(chunk.pageId)) && keywords.some((word) => contentLower.includes(word));
      })
  ...
  // Fallback по имени документа: если врач явно просит конкретную книгу
  // (например Fitzpatrick), а текстовый скоринг молчит, возвращаем фрагменты
  // из страниц совпавшего документа вместо ложного "ничего нет".
  if (results.length === 0) {
    const matchedDocIds = allDocs
      .filter((doc) => {
        const name = String(doc.name || '').toLowerCase();
        if (wantsFitzpatrick && /(fitz|fitzpatrick|фитц|фитцпатрик)/i.test(name)) return true;
        return expandedKeywords.some((word) => word.length >= 3 && name.includes(word));
      })
      .map((doc) => doc.id);

    if (matchedDocIds.length > 0) {
      const matched = allChunks
        .filter((chunk) => matchedDocIds.includes(chunk.documentId) && Number.isFinite(Number(chunk.pageId)))
        .slice(0, limit)
        .map((chunk) => ({ content: chunk.content, score: 0.2, chunk }));
      results = matched;
    }
  }
