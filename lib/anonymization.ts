...
export function anonymizeText(text: string): string {
  ...
  // ВЕРХНИЙ РЕГИСТР и/или подчеркивание: КУНИКИНА Д.В. / МАЛЫШЕНКО_Е.О.
  result = result.replace(/(^|[\s,;:()_])[А-ЯЁ]{2,40}[_\s]+[А-ЯЁ]\.\s*[А-ЯЁ]\.(?=$|[\s,;:()._])/gu, '$1[ФИО]');
  // Ролевые подписи: "Врач: Иванов И.И.", "Фельдшер Петрова А.А.", "Водитель Сидоров С.С."
  result = result.replace(
    /(\b(?:Врач|Фельдшер|Водитель|Медсестра|Медбрат|Доктор|Санитар)\s*[:\-]?\s*)([А-ЯЁA-Z][А-ЯЁA-Zа-яёa-z-]{1,40}(?:[_\s]+[А-ЯЁA-Z]\.\s*[А-ЯЁA-Z]\.|\s+[А-ЯЁ][а-яё]{1,30}(?:\s+[А-ЯЁ][а-яё]{1,30})?))/gu,
    '$1[ФИО]'
  );
