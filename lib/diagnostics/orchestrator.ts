...
const IMAGE_TO_TEXT_SYSTEM_PROMPT = `You are helping prepare inputs for a physician consilium.
Describe only factual visual findings and text from attached medical-document images
(scans, reports, discharge summaries, lab forms). Do NOT diagnose and do NOT provide
clinical interpretation; provide only structured description of what is visible.
If the image contains typed or handwritten report text, transcribe it as accurately as possible.`;
...
export async function describeImagesAsFindings(
  ...
  const userContent = [
    { type: 'text' as const, text: 'Describe findings from attached images.' },
  ...
  if (call.error) {
    return {
      text: `[Unable to automatically process attached images: ${call.error}]`,
...
function buildSpecialtyOpinionSystemPrompt(specialty: string): string {
  ...
  return `You are specialist physician "${specialty}" in a virtual physician consilium.
You are included specifically because of your specialty profile. Provide differential diagnosis
from your specialty perspective, focusing on what your clinical lens adds, including specialty-
specific red flags where applicable. Do not restate general case info; provide only your
professional assessment and rationale.
...
const AMSC_SYNTHESIS_SYSTEM_PROMPT = `You are a moderator of a virtual physician consilium.
Multiple specialists independently assessed the same clinical case and their opinions converged
(low disagreement). Produce a concise consensus diagnosis with rationale, briefly noting where
the specialists align.
${JSON_HYPOTHESES_INSTRUCTION}`;

function buildAmscSynthesisUserContent(contextBlock: string, opinions: SpecialtyOpinion[]): string {
  const opinionsBlock = opinions
    .map((o) => `### Specialty opinion "${o.specialty}"\n${o.content}`)
    .join('\n\n---\n\n');
...
async function runAmscRound0(
  ...
  onProgress?.({ type: 'stage', message: 'Selecting specialties for this case...' });
  ...
  if (!escalatedToDebate) {
    onProgress?.({ type: 'stage', message: 'Specialty opinions converged, building consensus...' });
  ...
  const escalationReason = skepticFlaggedCritical
    ? `Skeptic (${AMSC_FIXED_SKEPTIC_SPECIALTY}) flagged a potential red flag: ${skepticVerdict.reasoning || 'no details provided'}`
    : `Specialty opinions diverged in substance, not just wording (disagreement ${disagreementScore.toFixed(2)} > ${ROUND_CONTINUATION_THRESHOLD})${disagreement.reasoning ? `: ${disagreement.reasoning}` : ''}`;
  ...
  if (onBeforeDebateEscalation) {
    ...
    if (!permission.allowed) {
      ...
      const escalation: EscalationDecision = {
        ...
        reason: `${amscRound.escalationReason || 'Specialty opinions diverged.'} ${
          permission.denialReason || 'Full debate analysis was not started.'
        }`,
      };
      return buildResult(finalDiagnosis, state, escalation, extraUsage);
    }
