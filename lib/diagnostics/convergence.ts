...
function calculateExactDisagreementScore(normalizedDiagnoses: string[]): { score: number; reasoning: string } {
  ...
  return {
    ...
    reasoning: 'Exact string comparison of labels (semantic clustering unavailable).',
  };
}

const SEMANTIC_CLUSTER_SYSTEM_PROMPT = `You help estimate agreement between specialist physicians.
You are given a numbered list of most likely diagnoses from different consilium participants.
Wording may differ (synonyms, different granularity, different word order), but may refer to the SAME
clinical condition; e.g., equivalent labels should NOT be treated as disagreement.
Group diagnosis numbers into clusters so each cluster contains only clinically equivalent diagnoses
(semantic equivalence, not exact wording).
Reply STRICTLY as one-line JSON, without markdown:
{"clusters":[[1,3],[2]],"reasoning":"brief clustering rationale"}`;
...
export async function calculateSemanticDisagreementScore(hypothesesSets: Hypothesis[][]): Promise<DisagreementResult> {
  ...
  if (new Set(normalized).size === 1) {
    return { score: 0, reasoning: 'Exact diagnosis wording match.', usage: ZERO_USAGE };
  }
...
export function selectFinalDiagnosis(hypothesesSets: Hypothesis[][]): Hypothesis {
  ...
  return {
    diagnosis: 'Unable to produce final diagnosis automatically',
    probability: 0,
    reasoning: 'No consilium role returned structured hypotheses. Manual physician review is required.',
  };
}
