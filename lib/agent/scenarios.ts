...
const protocolScenario: AgentScenario = {
  ...
  icon: '📝',
  featured: true,
  keywords: ['протокол приёма', 'протокол приема', 'оформить приём', 'оформить прием', 'заключение приёма'],
  start: 'open',
  steps: {
    open: {
      id: 'open',
      message:
        'Открываю протокол приёма. Введите 2–3 фразы осмотра в подсвеченное поле и нажмите «Создать протокол».\n\nПодсказка: если вы пришли из готового разбора (ЭКГ, снимок), используйте на странице результата кнопку «В протокол» — текст подставится автоматически.',
      onEnter: [
