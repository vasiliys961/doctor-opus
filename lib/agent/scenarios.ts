  ...
  return {
    ...
    steps: {
      ...
      run: {
        id: 'run',
        message:
          'Выберите режим точности и запустите анализ: ⚡ Быстрый — скрининг, ⭐ Оптимизированный — рекомендуемый баланс, 🧠 С валидацией — для сложных случаев.\n\nПод готовым разбором есть кнопка «🔍 Проверить (Chain-of-Verification)» — независимая проверка результата на пропущенные альтернативы и red flags.',
...
const imageChooserScenario: AgentScenario = {
  ...
  steps: {
    ...
    run: {
      id: 'run',
      message:
        'Выберите режим точности и запустите анализ: ⚡ Быстрый, ⭐ Оптимизированный (рекомендуемый), 🧠 С валидацией.\n\nПосле результата смотрите блок «Релевантные источники»: он подбирает ссылки по вероятным диагнозам/находкам, а не только по типу изображения. Кнопка «🔍 Проверить (Chain-of-Verification)» под разбором даёт независимую проверку на пропущенные альтернативы и red flags.',
      options: [{ label: '✅ Получил результат', next: 'next' }],
    },
    next: buildTransferStep(),
    done: DONE_STEP,
...
/**
 * Консилиум — "Глубокий анализ" (мультиагентный MAI-DxO-стиль разбор внутри чата).
 * Сначала независимые мнения нескольких специальностей + скептик по red flags (дёшево),
 * при расхождении мнений — автоматическая эскалация в полный цикл дебатов из 5 ролей.
 * Премиум-функция.
 */
const consiliumScenario: AgentScenario = {
  id: 'consilium',
  title: 'Консилиум (глубокий анализ)',
  icon: '🩺',
  featured: true,
  keywords: ['консилиум', 'глубокий анализ', 'глубокий разбор', 'мультиагент', 'несколько мнений', 'второе мнение'],
  start: 'intro',
  steps: {
    intro: {
      id: 'intro',
      message:
        '«Консилиум» — режим глубокого анализа внутри ИИ-Ассистента для сложных мультисистемных кейсов. Как это работает:\n1) Сначала несколько профильных специальностей (подобранных под ваш кейс) + отдельный «скептик» по red flags высказываются независимо — это дёшево и быстро.\n2) Если их мнения совпадают — сразу выдаётся консенсус.\n3) Если расходятся — автоматическая эскалация в полный цикл дебатов из 5 ролей (гипотезы → выбор обследований → критик → контроль избыточности → чек-лист).\n4) При низкой уверенности в тяжёлом диагнозе кейс помечается для очной проверки врачом.\n\nЭто премиум-функция. Опишите кейс текстом (жалобы, анамнез, вложенные заключения).',
      onEnter: [
        { type: 'navigate', href: '/chat' },
        {
          type: 'highlight',
          selector: '[data-tour="chat-question-input"]',
          title: 'Включите «Консилиум»',
          text: 'Над полем ввода включите тумблер «Консилиум», затем опишите кейс здесь и отправьте.',
          on: 'top',
        },
      ],
      options: [{ label: 'Готово', next: 'done' }],
...
const advancedScenario: AgentScenario = {
  ...
  icon: '🔬',
  keywords: ['клиническ разбор', 'разбор случа', 'сложн случа', 'разбор пациент'],
  start: 'intro',
  steps: {
    intro: {
      id: 'intro',
...
const consultScenario: AgentScenario = {
  ...
  steps: {
    ask: {
      id: 'ask',
      message:
        'Открываю ИИ-Ассистента. Сформулируйте вопрос в подсвеченном поле.\n\nМожно выбрать профильную специальность и режим RAG. RAG (Retrieval-Augmented Generation) — это ответ с опорой на источники: система сначала находит релевантные фрагменты в клинических рекомендациях и вашей загруженной литературе, а затем отвечает на их основе со ссылками. Часть ответа, которая выходит за рамки найденных источников, модель обязана явно пометить «⚠️ не подтверждено источниками» — так видно, где обоснованный ответ, а где общие знания модели.\n\nПод каждым ответом есть кнопка «🔍 Проверить ответ (Chain-of-Verification)» — отдельный независимый вызов модели-скептика, который ищет пропущенные альтернативы и red flags в уже готовом ответе.\n\nЕсли кейс выглядит сложным, для описания случая (без снимка) советую режим «Консилиум» (несколько мнений + эскалация при разногласиях).',
      onEnter: [
        { type: 'navigate', href: '/chat' },
        {
          type: 'highlight',
