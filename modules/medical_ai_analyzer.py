...
class EnhancedMedicalAIAnalyzer:
    ...
    def __init__(self, api_key: str):
        ...
        # Для клинического визуального анализа сразу используем Opus 4.5,
        # без промежуточных Sonnet-моделей.
        self.models = [
...
""",
            ...
            ImageType.XRAY: """
Ты — профессор рентгенологии и профессионал высшей категории по интерпретации рентгенограмм опорно‑двигательного аппарата и грудной клетки.
Систематически проанализируй рентгенограмму как для коллеги‑ортопеда/травматолога/терапевта:
...
4. ДИФФЕРЕНЦИАЛЬНАЯ ДИАГНОСТИКА И КЛИНИЧЕСКОЕ ЗАКЛЮЧЕНИЕ:
   - Сформулируй основной клинический диагноз по результатам рентгенографии (с указанием стороны, сустава/сегмента, стадии/тяжести процесса).
   - Укажи ключевые дифференциальные диагнозы, которые следует учитывать.
   - Явно отметь, если по данным рентгена диагностировать заболевание невозможно (и почему).

5. СРОЧНОСТЬ И ТАКТИКА:
   - Экстренные находки (угроза жизни, необходимость немедленной госпитализации/операции).
   - Плановые изменения (хронические дегенеративные процессы, такие как коксартроз I–II стадии и т.п.).
   - Рекомендации по дообследованию (МРТ, КТ, контрольные рентгенограммы, лаборатория).

Ответ СТРОГО в JSON формате, используя структуру diagnosis.primary_diagnosis как основное клиническое заключение по снимку.
""",
...
"""
    ...
    def analyze_image(self, image_array: np.ndarray, image_type: Optional[ImageType] = None, 
        ...
        try:
            ...
            # Универсальный профиль профессора‑диагноста для всех типов (рентген, КТ, МРТ, УЗИ, ЭКГ и др.)
            global_header = (
                "Ты — профессор клинической медицины и эксперт по визуальной диагностике "
                "(рентген, КТ, МРТ, УЗИ, ЭКГ и другие методы). Консультируешь только врачей "
                "и формулируешь клинически значимые, практичные заключения."
            )
            base_prompt = global_header + "\n\n" + base_prompt
