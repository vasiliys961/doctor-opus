...
try:
    # Профессор (Opus) из основного ассистента
    ...
    if uploaded_files and len(uploaded_files) >= 2:
        ...
        if st.button("🔄 Выполнить сравнительный анализ"):
            ...
            if results:
                ...
                # ИИ-заключение по сравнению — формирует профессор (Opus) с клинической директивой
                st.subheader("🤖 Заключение профессора по сравнительному анализу (Opus)")
                
                if OpenRouterAssistant is None:
                    st.error("❌ Класс OpenRouterAssistant недоступен. Проверьте файл `claude_assistant.py`.")
                else:
                    with st.spinner("Профессор клинической медицины (Opus) анализирует сравнительные данные..."):
                        
                        # Формируем текстовый контекст для профессора: без картинок, только с результатами
                        context_parts = []
                        context_parts.append(f"Тип сравнения: {comparison_type}")
                        context_parts.append(f"Количество изображений: {len(results)}")
                        context_parts.append("")
                        
                        for i, result in enumerate(results, 1):
                            findings = result.structured_findings or {}
                            primary_diag = findings.get("diagnosis", {}).get("primary_diagnosis", "Не определен")
                            diff_diag = findings.get("diagnosis", {}).get("differential_diagnosis", [])
                            icd10 = findings.get("diagnosis", {}).get("icd10_codes", [])
                            risk = findings.get("risk_assessment", {}).get("urgency_level", "планово")
                            
                            # В контекст для профессора не тянем технические поля и типы (dermatoscopy/xray и т.п.),
                            # только клинически значимые вещи.
                            context_parts.append(f"Исследование {i}: файл {result.filename}")
                            context_parts.append(f"- Итоговый диагноз автоанализа: {primary_diag}")
                            context_parts.append(f"- Срочность (оценка автоанализа): {risk}")
                            if diff_diag:
                                context_parts.append(f"- Возможные дифференциальные диагнозы: {', '.join(diff_diag)}")
                            if icd10:
                                context_parts.append(f"- Коды МКБ-10: {', '.join(icd10)}")
                            context_parts.append("")
                        
                        professor_context = "\n".join(context_parts)
                        
                        professor_question = (
                            "Сравните представленные визуализационные исследования между собой во времени и/или по проекциям. "
                            "Игнорируйте технические детали и внутренние типы (dermatoscopy/xray и т.п.) — ориентируйтесь только на клинический смысл. "
                            "Дайте клиническую директиву для врача: динамика патологического процесса, изменение диагноза/стадии, тактика ведения, "
                            "необходимость дообследования и наблюдения."
                        )
                        
                        try:
                            professor = OpenRouterAssistant()
                            comparative_analysis = professor.get_response(
                                professor_question,
                                context=professor_context,
                                use_sonnet_4_5=False  # приоритет Opus
                            )
                            ...
                            st.error(f"Ошибка генерации сравнительного анализа профессором: {e}")
            else:
