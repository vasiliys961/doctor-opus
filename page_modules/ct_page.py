...
def show_ct_analysis():
    ...
    with st.expander("💡 Полезные подсказки", expanded=True):
        ...
        **💡 Советы по использованию режимов анализа:**
        - **⚡ Быстрый анализ (Gemini Flash)** — двухэтапный скрининг (сначала краткое структурированное описание КТ‑исследования, затем текстовый разбор), даёт компактное заключение и общий сигнал риска, удобен для первичного просмотра и триажа.
    ...
    try:
        ...
        combined_context = st.text_area(
            ...
            value="",
            placeholder="Например: боли в животе в течение 2 дней, тошнота, повышение температуры, подозрение на острый аппендицит...",
            help="Эта информация поможет ИИ дать более точное и персонализированное заключение.",
            key="ct_manual_context"
        )
        
        st.markdown("---")
        
        specialist_info = get_specialist_info(ImageType.CT) if get_specialist_info else {'role': 'Врач-радиолог', 'experience': 'многолетним опытом'}
        if SPECIALIST_DETECTOR_AVAILABLE and get_specialist_prompt:
            ...
            # Fallback промпт для КТ - используем централизованный детальный промпт
            try:
                ...
                prompt = get_ct_diagnostic_prompt(system_prompt)
                ...
                prompt = f"Проанализируйте КТ-снимок как {specialist_info['role']} с {specialist_info['experience']}. Оцените структуры, патологические изменения, денситометрию."
        
        # Отображение сохраненных результатов анализа (если есть)
        gemini_result = st.session_state.get('ct_gemini_result', '')
        opus_result = st.session_state.get('ct_opus_result', '')
        ai_result = st.session_state.get('ct_ai_result', '')
        ...
        if gemini_result or opus_result or ai_result:
            ...
            if opus_result:
                opus_timestamp = st.session_state.get('ct_opus_timestamp', '')
                st.markdown(f"#### 🎯 Точный анализ (Opus 4.5){f' - {opus_timestamp}' if opus_timestamp else ''}")
                st.write(opus_result)
                st.markdown("---")
            
            if ai_result:
                ai_timestamp = st.session_state.get('ct_ai_timestamp', '')
        ...
        with col_fast:
            if st.button("⚡ Быстрый анализ (Gemini Flash)", use_container_width=True, type="primary", key="ct_fast"):
                if image_array is None:
                    st.warning("⚠️ Сначала загрузите КТ-снимок.")
                else:
                    try:
                        # Шаг 1: Gemini Vision — структурированное описание КТ
                        desc_prompt = (
                            "Ты — профессиональный врач‑радиолог по КТ, консультирующий только коллег‑клиницистов.\n"
                        ...
                        with st.spinner("📷 Gemini Vision описывает КТ..."):
                            ct_description = assistant.send_vision_request_gemini_fast(desc_prompt, image_array, str(metadata))

                        if not isinstance(ct_description, str):
                            ct_description = str(ct_description)

                        st.session_state.ct_vision_description = ct_description
                        ...
                        text_context = (
                            f"{context_header}Ниже приведено текстовое описание КТ‑исследования, автоматически полученное "
                            ...
                            "=== ОПИСАНИЕ КТ ОТ GEMINI VISION ===\n"
                            f"{ct_description}\n"
                        )
                        user_message = (
                            "На основе приведённого выше описания КТ‑исследования выполни экспертный анализ и сформируй "
                        ...
                        with st.spinner("🧠 Gemini Flash формирует клиническую директиву по КТ..."):
                        ...
                        st.error(f"❌ Ошибка быстрого анализа КТ (Gemini двухэтапный): {e}")
        ...
        with col_precise:
            ...
            if st.button(f"🎯 Точный анализ (Opus 4.5) - на {accuracy_diff}% точнее", use_container_width=True, type="primary", key="ct_precise"):
                ...
                if result:
                    ...
                    st.session_state.ct_opus_result = result
                    st.session_state.ct_opus_timestamp = timestamp
                    st.session_state.ct_analysis_result = result
                    st.session_state.ct_analysis_timestamp = timestamp
                    st.rerun()
        ...
        consultant_model = st.radio(
            ...
            horizontal=False,
            key="ct_consultant_model"
        )
        
        ct_vision_description = st.session_state.get("ct_vision_description")
        gemini_result = st.session_state.get("ct_gemini_result")
        opus_result = st.session_state.get("ct_opus_result")
        
        consultant_button = st.button("🧠 Итоговое заключение ИИ‑консультанта по результатам анализа", use_container_width=True, key="ct_consultant_final")
        st.caption("💰 ≈2 ед. (Sonnet) / ≈3–4 ед. (Opus)")
        
        if consultant_button:
            if not (ct_vision_description or gemini_result or opus_result):
                st.warning("⚠️ Сначала выполните анализ КТ (Gemini и/или Opus), чтобы получить описание и заключения.")
            else:
                try:
                    parts = []
                    if ct_vision_description:
                        parts.append("=== ТЕКСТОВОЕ ОПИСАНИЕ КТ ОТ VISION-МОДЕЛИ ===\n" + str(ct_vision_description))
                    if gemini_result:
                    ...
                    context_header = f"КЛИНИЧЕСКИЙ КОНТЕКСТ ПАЦИЕНТА:\n{combined_context}\n\n" if combined_context else ""
                    text_context = (
                        f"{context_header}Ниже приведено текстовое описание КТ‑исследования и заключения разных моделей "
                    ...
                    user_message = (
                        "Ты — опытный врач-радиолог‑консультант по КТ. "
                        ...
                        "2) ПОДРОБНЫЕ КТ‑НАХОДКИ (только клинически значимые):\n"
                        "   - Локализация, размеры и характер очагов/образований, состояние органов, признаки острой патологии и др.\n"
                        ...
                        "   - Б) Тактика в стационаре / амбулаторно: какие дообследования нужны (МРТ, УЗИ, анализы и т.п.).\n"
                    ...
                    timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
                    st.session_state.ct_assistant_like_result = professor_response
                    st.session_state.ct_assistant_like_timestamp = timestamp
                    st.session_state.ct_ai_result = professor_response
                    st.session_state.ct_ai_timestamp = timestamp
                    # Также обновляем общий результат для экспорта
                    st.session_state.ct_analysis_result = professor_response
                    st.session_state.ct_analysis_timestamp = timestamp
                except Exception as e:
        ...
        if 'ct_analysis_result' in st.session_state and st.session_state.ct_analysis_result:
            ...
            result_text = st.session_state.ct_analysis_result
            timestamp = st.session_state.get('ct_analysis_timestamp', '')
            
            col1, col2 = st.columns(2)
            with col1:
                try:
                    from utils.word_report_generator import generate_word_report, get_word_report_filename
                    word_bytes = generate_word_report('CT', result_text, timestamp=timestamp)
                    if word_bytes:
                        st.download_button(
                            label="📥 Скачать заключение (.docx)",
                            data=word_bytes,
                            file_name=get_word_report_filename('CT', timestamp),
                            mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                            key="download_ct_word"
                        )
                except Exception:
                    st.info("💡 Установите python-docx для экспорта в Word")
            with col2:
                header = f"Заключение по КТ\nВремя анализа: {timestamp}" if timestamp else "Заключение по КТ"
                report_text = f"{header}\n\n{result_text}"
                st.download_button(
                    label="📥 Скачать заключение (.txt)",
                    data=report_text,
                    file_name=f"CT_report_{timestamp.replace(' ', '_').replace(':', '-') if timestamp else 'latest'}.txt",
                    mime="text/plain",
                    key="download_ct_txt"
                )
