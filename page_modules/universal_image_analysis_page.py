...
def show_universal_image_analysis():
    ...
    try:
        ...
        combined_context = st.text_area(
            ...
            value="",
            placeholder="Введите клинические данные для более точного анализа...",
            help="Эта информация поможет ИИ дать более точное и персонализированное заключение.",
            key="universal_manual_context"
        )
        ...
        with col_fast:
            ...
            if st.button("⚡ Быстрый анализ (Gemini Flash)", use_container_width=True, type="primary", key="universal_fast"):
                try:
                    ...
                    text_context = (
                        f"{context_header}Ниже приведено текстовое описание медицинского изображения, автоматически полученное "
