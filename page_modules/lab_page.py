...
def show_lab_analysis():
    ...
    with st.expander("💡 Полезные подсказки", expanded=True):
        ...
        **💡 Советы по использованию:**
        - Поддерживаются форматы: PDF, Excel (XLSX, XLS), CSV, JSON, XML, TXT
        - Система автоматически извлекает данные из различных форматов
        - Референсные значения сопоставляются автоматически
        - Критические отклонения выделяются автоматически
        - Результаты можно экспортировать в Excel для дальнейшей работы
        """)
    ...
    combined_context = st.text_area(
        "Введите сведения о пациенте (жалобы, анамнез, симптомы, принимаемые препараты):",
        value="",
        placeholder="Например: слабость, головокружение, диета с низким содержанием железа, принимает препараты магния...",
        help="Эта информация поможет ИИ дать более точное и персонализированное заключение по анализам.",
        key="lab_manual_context"
    )
...
"""
                            ...
                            base_prompt = f"""{context_header}Проанализируйте лабораторные результаты как врач-лаборант-консультант с 15-летним опытом работы в клинической лаборатории. 
