...
def show_mri_analysis():
    ...
    try:
        ...
        combined_context = st.text_area(
            ...
            value="",
            placeholder="Например: головные боли в течение месяца, слабость в левой руке, судорожный приступ в анамнезе...",
            help="Эта информация поможет ИИ дать более точное и персонализированное заключение.",
            key="mri_manual_context"
        )
            ...
            if st.button("⚡ Быстрый анализ (Gemini Flash)", use_container_width=True, type="primary", key="mri_fast"):
                if image_array is None:
                    ...
                    try:
                        ...
                        text_context = (
                            f"{context_header}Ниже приведено текстовое описание МРТ‑исследования, автоматически полученное "
        ...
        if consultant_button:
            if not (mri_vision_description or gemini_result or opus_result):
                ...
                try:
                    ...
                    context_header = f"КЛИНИЧЕСКИЙ КОНТЕКСТ ПАЦИЕНТА:\n{combined_context}\n\n" if combined_context else ""
                    text_context = (
                        f"{context_header}Ниже приведено текстовое описание МРТ‑исследования и заключения разных моделей "
