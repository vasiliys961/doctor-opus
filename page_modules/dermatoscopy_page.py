...
def show_dermatoscopy_analysis():
    ...
    try:
        ...
        combined_context = st.text_area(
            ...
            value="",
            placeholder="Например: новообразование на коже спины, появилось полгода назад, быстро растет, зуд, кровоточивость...",
            help="Эта информация поможет ИИ дать более точное и персонализированное заключение.",
            key="derma_manual_context"
        )
            ...
            if st.button("⚡ Быстрый анализ (Gemini Flash)", use_container_width=True, type="primary", key="derma_fast"):
                if image_array is None:
                    ...
                    try:
                        ...
                        text_context = (
                            f"{context_header}Ниже приведено текстовое описание дерматоскопического изображения, автоматически полученное "
