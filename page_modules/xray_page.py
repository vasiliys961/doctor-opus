...
def show_xray_analysis():
    ...
    try:
        ...
        combined_context = st.text_area(
            ...
            value="",
            placeholder="Например: кашель в течение недели, температура 38.5, одышка, подозрение на пневмонию...",
            help="Эта информация поможет ИИ дать более точное и персонализированное заключение.",
            key="xray_manual_context"
        )
            ...
            if st.button("⚡ Быстрый анализ (Gemini Flash)", use_container_width=True, type="primary", key="xray_fast"):
                if image_array is None:
                    ...
                    try:
                        ...
                        text_context = (
                            f"{context_header}Ниже приведено текстовое описание рентгеновского снимка грудной клетки, автоматически полученное "
        ...
        if st.button("🧠 Итоговое заключение ИИ‑консультанта по результатам анализа", use_container_width=True, key="xray_consultant_final"):
            if not (xray_vision_description or gemini_result or opus_result):
                ...
                try:
                    ...
                    context_header = f"КЛИНИЧЕСКИЙ КОНТЕКСТ ПАЦИЕНТА:\n{combined_context}\n\n" if combined_context else ""
                    text_context = (
                        f"{context_header}Ниже приведено текстовое описание рентгеновского снимка и заключения разных моделей "
