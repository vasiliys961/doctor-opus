...
🚨 **КРИТИЧЕСКИ ВАЖНО - ПЕРВЫМ ДЕЛОМ ПРОВЕРЬ НА ЖИЗНЕУГРОЖАЮЩИЕ АРИТМИИ!**
Перед любым другим анализом ОБЯЗАТЕЛЬНО проверь наличие фибрилляции желудочков, асистолии или желудочковой тахикардии без пульса.
Если видишь фибрилляцию желудочков - это ПРИОРИТЕТ #1, укажи это ПЕРВЫМ в заключении!
...
**1. ФИБРИЛЛЯЦИЯ ЖЕЛУДОЧКОВ (ЖФ) - ЖИЗНЕУГРОЖАЮЩЕЕ СОСТОЯНИЕ!**
   ...
   - **НЕ ПУТАТЬ с другими состояниями:**
     * **ЖФ vs Мерцательная аритмия:** ЖФ - нет организованных QRS, хаотичные волны > 300 уд/мин; Мерцательная аритмия - есть организованные QRS комплексы, частота желудочков 100-180 уд/мин
     * **ЖФ vs Желудочковая тахикардия:** ЖФ - нет организованных QRS, хаотичные волны; ЖТ - есть организованные широкие QRS комплексы, регулярные, частота 150-250 уд/мин
     * **ЖФ vs Асистолия:** ЖФ - есть электрическая активность (хаотичные волны); Асистолия - полное отсутствие электрической активности (прямая линия)
   ...
   - **ОБЯЗАТЕЛЬНО УКАЖИ:** "НЕОТЛОЖНОЕ СОСТОЯНИЕ, ТРЕБУЕТ НЕМЕДЛЕННОЙ ДЕФИБРИЛЛЯЦИИ!" если видишь фибрилляцию желудочков

**2. АСИСТОЛИЯ:** полное отсутствие электрической активности — НЕМЕДЛЕННАЯ РЕАНИМАЦИЯ!

**3. ЖЕЛУДОЧКОВАЯ ТАХИКАРДИЯ БЕЗ ПУЛЬСА:** широкие QRS > 150 уд/мин — НЕМЕДЛЕННАЯ ДЕФИБРИЛЛЯЦИЯ!
...
{prompt}"""
            ...
            try:
                ...
                # Fallback на детальный промпт если модуль недоступен
                medical_prompt = f"""{self.system_prompt}
...
"""
        ...
        if claude_failed and fallback_models:
            print(f"🔄 [FALLBACK] Все Claude модели недоступны. Пробую fallback модели: {', '.join([self._get_model_name(m) for m in fallback_models])}")
    ...
    def send_vision_request_gemini_fast(self, prompt: str, image_array=None, metadata=None):
        ...
        # Формируем промпт (используем полный детальный промпт как у Opus, но без system_prompt и без обращений к коллегам)
...
2. **КЛЮЧЕВЫЕ НАХОДКИ (только клинически значимые):**
   - Для грудной клетки: легочные поля, корни легких, средостение, сердце, диафрагма, костные структуры
   - Для конечностей: костные структуры, суставы, мягкие ткани
   - Для таза: костные структуры, тазобедренные суставы

3. **ОПРЕДЕЛЕНИЕ ЭНДОПРОТЕЗОВ И ИМПЛАНТОВ:**
   - ОБЯЗАТЕЛЬНО определи ТИП сустава ПЕРЕД описанием
   - Тазобедренный эндопротез = вертлужный компонент в тазу + бедренный компонент в бедре
   - Плечевой эндопротез = гленоидный компонент на лопатке + плечевой компонент в плечевой кости
   - НЕ ПУТАТЬ эти типы!

4. **ПАТОЛОГИЧЕСКИЕ ИЗМЕНЕНИЯ:**
   - Переломы (тип, локализация, смещение)
   - Деструктивные изменения
   - Дегенеративные изменения
   - Воспалительные изменения
   - Опухолевые изменения
...
- НИКОГДА не путай локализации! СНАЧАЛА определи ТОЧНО, что видишь на снимке
- Если видишь металлические конструкции - ОБЯЗАТЕЛЬНО опиши их детально
- Указывай ТОЧНУЮ локализацию для каждого изменения
- Не используй общие фразы - будь конкретен
- Если что-то не видно четко - так и укажи, не выдумывай
...
{prompt}"""
    ...
    def send_vision_request_streaming(self, prompt: str, image_array=None, metadata=None):
        """Анализ изображения с streaming через Opus 4.5 - текст появляется постепенно
        ...
        if "экг" in prompt_lower or "ecg" in prompt_lower:
            # Используем полный промпт с детальными инструкциями по критическим состояниям
            try:
                from prompts.diagnostic_prompts import get_ecg_diagnostic_prompt
                medical_prompt = get_ecg_diagnostic_prompt(self.system_prompt)
...
{prompt}"""
                        ...
                        if line_text.startswith('data: '):
                            ...
                            try:
                                ...
                                if 'choices' in data and len(data['choices']) > 0:
                                    ...
                                    if content:
                                        tokens_received += len(content.split())  # Примерная оценка
                                        yield content
                                    
                                    # Проверяем usage в финальном сообщении
                                    if 'usage' in data:
    ...
    def _send_vision_request_streaming_fallback(self, prompt: str, image_array=None, metadata=None, fallback_model: str = "anthropic/claude-sonnet-4.5"):
        ...
        # Используем упрощенный промпт для fallback
        prompt_lower = prompt.lower() if prompt else ""
        medical_prompt = f"""{self.system_prompt}
...
{prompt}"""
    ...
    def get_response_streaming(self, user_message: str, context: str = "", use_sonnet_4_5: bool = False, force_opus: bool = False):
        ...
        Args:
            ...
            force_opus: Принудительно использовать Opus 4.5 (гарантированный Opus без fallback)
