...
{prompt}"""
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Логируем завершение streaming с информацией о токенах
                model_name = self._get_model_name(model)
                if tokens_received > 0:
                    print(f"✅ [OPUS 4.5 STREAMING] Завершено. Модель: {model_name}, использовано токенов: {tokens_received}")
                else:
                    print(f"✅ [OPUS 4.5 STREAMING] Завершено. Модель: {model_name}")
                ...
                log_api_call(model, False, 0, error_msg)
                track_model_usage(model, False)
                print(f"❌ [OPUS 4.5 STREAMING] {error_msg}")
            ...
            log_api_call(model, False, 180, error_msg)
            track_model_usage(model, False)
            print(f"❌ [OPUS 4.5 STREAMING] {error_msg}")
            ...
            log_api_call(model, False, 0, error_msg)
            track_model_usage(model, False)
            print(f"❌ [OPUS 4.5 STREAMING] Ошибка: {error_msg}")
        ...
        for model in models_to_try:
            try:
                ...
                if response.status_code == 200:
                    ...
                    for line in response.iter_lines():
                        if line:
                            ...
                            if line_text.startswith('data: '):
                                data_str = line_text[6:]  # Убираем "data: "
                                if data_str.strip() == '[DONE]':
                                    print(f"✅ [{model_name} STREAMING] Streaming завершен, получено токенов: ~{tokens_received}")
                                    break
                                try:
                                    data = json.loads(data_str)
                                    if 'choices' in data and len(data['choices']) > 0:
                    ...
                    track_model_usage(model, False)
                    model_name = self._get_model_name(model)
                    print(f"❌ [{model_name} STREAMING] {error_msg}. Пробую следующую модель...")
