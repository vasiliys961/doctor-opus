...
."""
    ...
    def send_vision_request(self, prompt: str, image_array=None, metadata=None, use_cache: bool = False, 
        ...
        # Это нужно для правильного ключа кеша при чтении и сохранении
        ...
        if use_cache and image_array is not None and not is_ecg:
            ...
            # Определяем первую модель (та же логика что и models_to_try[0])
            if force_model:
        ...
        if use_cache and image_array is not None and not is_ecg:
            ...
            cache_key = get_cache_key(prompt, image_hash, primary_model_for_cache)  # Исправлено: используем primary_model_for_cache
            cached_result = get_cached_result(cache_key, max_age_hours=24)  # Добавлена проверка возраста
            ...
            for max_tokens in max_tokens_list:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if use_cache and image_array is not None and not is_ecg:
                            image_hash = get_image_hash(image_array)
                            cache_key = get_cache_key(prompt, image_hash, primary_model_for_cache)  # Исправлено: используем primary_model_for_cache вместо model
                            save_to_cache(cache_key, result, max_age_hours=24)  # Явно указан max_age_hours
