...
_diagnostic_prompts_cache = None
...
def _get_diagnostic_prompts():
    """Ленивая загрузка диагностических промптов"""
    global _diagnostic_prompts_cache
    if _diagnostic_prompts_cache is None:
        try:
            ...
            _diagnostic_prompts_cache = {
                ...
                'xray': get_xray_diagnostic_prompt,
            ...
            _diagnostic_prompts_cache = {}
    return _diagnostic_prompts_cache
...
."""
    ...
    def send_vision_request(self, prompt: str, image_array=None, metadata=None, use_cache: bool = False, 
        ...
        if scan_only_mode:
            ...
            # Используем детальный диагностический промпт для ЭКГ из отдельного файла
            ...
            if 'ecg' in diagnostic_prompts:
                ...
                # Fallback на старый промпт, если файл не загрузился
...
Не придумывай диагноз при отсутствии достаточных признаков."""
            ...
            # Используем детальный диагностический промпт из отдельного файла
            diagnostic_prompts = _get_diagnostic_prompts()
...
"""
            ...
            # Используем детальный диагностический промпт для МРТ
            diagnostic_prompts = _get_diagnostic_prompts()
...
Вы — врач-нейрорадиолог с 20-летним опытом работы. Выполните полный структурированный анализ в формате «Клиническая директива»."""
        
        elif "кт" in prompt_lower or "ct" in prompt_lower or "компьютерн" in prompt_lower:
            # Используем детальный диагностический промпт для КТ
            diagnostic_prompts = _get_diagnostic_prompts()
...
Ты — профессиональный радиолог. Анализируй изображение, выявляй патологические изменения, давай заключение согласно международным стандартам (ESR, ACR, Fleischner Society, RSNA)."""
...
"""
            ...
            # Используем детальный диагностический промпт для УЗИ
            diagnostic_prompts = _get_diagnostic_prompts()
...
Вы — врач ультразвуковой диагностики с 12-летним стажем работы. Детально опишите УЗИ-картину в формате «Клиническая директива»."""
        
        elif "дерматоскопия" in prompt_lower or "дерматоскоп" in prompt_lower or "dermatoscopy" in prompt_lower:
            # Используем детальный диагностический промпт для дерматоскопии
            diagnostic_prompts = _get_diagnostic_prompts()
...
Ты — дерматоонколог с 15+ годами опыта. Проведи дерматоскопический анализ и дай ТОЧНЫЙ ДИАГНОЗ с оценкой риска меланомы в формате «Клиническая директива»."""
