...
3. ПОДРОБНЫЙ АНАЛИЗ ИНТЕРВАЛОВ И ЗУБЦОВ:
   - Интервал PR (мс): численное значение, норма/укорочен/удлинён.
   - Комплекс QRS (мс): длительность, ось, амплитуда, морфология.
   - Интервал QT и QTc (мс): указать метод расчёта QTc (Bazett/Fredericia), норма/удлинён/укорочен.
   - Электрическая ось сердца: численное значение в градусах и классификация (норма, отклонение влево/вправо).
   - Зубец P: наличие, форма, амплитуда, длительность, признаки гипертрофии предсердий.
   - Комплекс QRS: наличие патологических Q‑зубцов (ширина > 40 мс, глубина > 25 % R), зоны потери R‑прогрессии.
   - Сегмент ST: элевация/депрессия (в мм) в каждом важном отведении, форма (горизонтальная, косонисходящая, куполообразная),
     типичные паттерны ишемии/инфаркта.
   - Зубец T: полярность, амплитуда, инверсия, двухфазность.
   - Дополнительные волны: U‑зубцы, ε‑волны и другие редкие находки.

4. АРИТМИИ И НАРУШЕНИЯ РИТМА (ФОКУС‑АНАЛИЗ):
   - Чётко классифицируй выявленные нарушения ритма:
     * Наджелудочковые: фибрилляция предсердий, трепетание предсердий, AV‑реципрокные и фокальные тахикардии, частые/редкие наджелудочковые экстрасистолы.
     * Желудочковые: одиночные/парные/залповые желудочковые экстрасистолы, желудочковая тахикардия (устойчивая/неустойчивая), bidirectional VT и др.
   - Определи наличие пауз, синоатриальной блокады, синдрома слабости синусового узла.
   - Укажи клинически значимые паттерны высокого риска (например, быстрая ФП при WPW, полиморфная ЖТ, Brugada‑подобный паттерн, удлинённый QT).
   - ОТДЕЛЬНО ПРОВЕРЬ критерии фибрилляции предсердий:
     * Полное отсутствие чётких P‑зубцов перед каждым QRS во всех отведениях.
     * Нерегулярно нерегулярные RR‑интервалы без повторяющегося паттерна.
     Если оба критерия выполняются — прямо укажи, что это фибрилляция предсердий (с комментарием по вероятной форме, если возможно).
     Если критерии частично, опиши это как подозрение на ФП и предложи дополнительные обследования (длинная запись, Холтер и т.п.).

5. ПАТОЛОГИЧЕСКИЕ ИЗМЕНЕНИЯ И ГИПЕРТРОФИЯ:
   - Признаки гипертрофии ЛЖ/ПЖ, предсердий (с указанием критериев).
   - Признаки острой ишемии/инфаркта миокарда (локализация по отведениям, вероятное время — острый/подострый/хронический).
   - Возможные электролитные и лекарственные эффекты (например, дигиталис, гипо-/гиперкалиемия, удлинение QT на фоне препаратов).

6. КЛИНИЧЕСКОЕ ЗАКЛЮЧЕНИЕ В ФОРМАТЕ «КЛИНИЧЕСКАЯ ДИРЕКТИВА»:
   1) Клинический обзор (2–3 предложения: суть картины и уровень срочности).
   2) Дифференциальный диагноз и наиболее вероятный основной диагноз с указанием кодов ICD‑10.
   3) План действий (step‑by‑step):
      - Неотложные действия (если нужны): госпитализация, мониторинг, купирование аритмии.
      - Дальнейшая диагностика: ЭХОКГ, холтер, нагрузочные тесты, лабораторные маркеры и т.п.
      - Терапия и коррекция сопутствующих состояний (без выдуманных дозировок; дозы указывать только если они общеприняты).
      - Мониторинг и «красные флаги» для врача.
