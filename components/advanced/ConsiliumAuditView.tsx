  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-4 sm:p-6 mb-4 border border-primary-100">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-4">
        <h3 className="font-bold text-lg text-primary-900">🩺 {i18n.consiliumResultTitle}</h3>
        <span className="text-xs bg-teal-50 text-teal-700 px-2 py-1 rounded border border-teal-100 font-bold w-fit">
          💰 {result.totalCostUsd.toFixed(3)} $ · {result.totalTokensUsed.toLocaleString(locale)} {i18n.consiliumTokensUnit}
        </span>
      </div>

      {result.requiresHumanReview && (
        <div className="mb-4 bg-amber-50 border border-amber-300 rounded-lg px-4 py-3 text-amber-900">
          <div className="font-bold text-sm mb-1">⚠️ {i18n.consiliumReviewRequired}</div>
      ...
      <div className="mb-4 bg-primary-50 border border-primary-200 rounded-lg px-4 py-3">
        <div className="text-xs font-semibold text-primary-700 mb-1">{i18n.consiliumFinalDiagnosis}</div>
        <div className="font-bold text-primary-900">
          {result.finalDiagnosis.diagnosis}
          <span className="ml-2 text-xs font-normal text-primary-600">
            ({i18n.consiliumProbabilityLabel} {(result.finalDiagnosis.probability * 100).toFixed(0)}%)
          </span>
        </div>
      ...
      <div className="mb-4 text-xs text-slate-600 flex items-center gap-2">
        <span className="font-semibold">{i18n.consiliumAgentDisagreement}</span>
        <span
        ...
        <span>· {i18n.consiliumDebateRounds} {result.auditTrail.roundOutputs.length}</span>
      </div>
      ...
      {amscRound && (
        <div className="mb-4 border border-slate-200 rounded-lg overflow-hidden">
          ...
          >
            <span>
              {i18n.consiliumRoundZeroSpecialties} ({amscRound.selectedSpecialties.length}){' '}
              <span
              ...
              >
                {amscRound.escalatedToDebate ? i18n.consiliumEscalatedToDebate : i18n.consiliumNoEscalation}
              </span>
            </span>
          ...
          {amscExpanded && (
            <div className="p-4 space-y-3">
                ...
                <div key={opinion.specialty} className="border-b border-slate-100 last:border-0 pb-3 last:pb-0">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-[10px] text-slate-400">
                      {opinion.model} · {opinion.promptTokens + opinion.completionTokens} tok.
                    </span>
                  </div>
                  {opinion.error ? (
              ...
              {amscRound.synthesisContent && (
                <div className="pt-1">
                  <div className="font-bold text-sm text-slate-800 mb-1">🤝 {i18n.consiliumConsensusSynthesis}</div>
        ...
        {result.auditTrail.roundOutputs.map((outputs, roundIdx) => (
          <div key={roundIdx} className="border border-slate-200 rounded-lg overflow-hidden">
            <button
              ...
              className="w-full flex items-center justify-between px-4 py-2 bg-slate-50 hover:bg-slate-100 text-sm font-semibold text-slate-700"
            >
              <span>{formatDiagnosticsTemplate(i18n.consiliumRoundHeader, { round: roundIdx + 1 })}</span>
                  ...
                  <div key={output.role} className="border-b border-slate-100 last:border-0 pb-3 last:pb-0">
                    <div className="flex items-center justify-between mb-1">
                      <span className="font-bold text-sm text-slate-800">
                        {ROLE_ICONS[output.role]} {roleLabels[output.role]}
                      </span>
                      <span className="text-[10px] text-slate-400">
                        {output.model} · {output.promptTokens + output.completionTokens} tok.
                      </span>
                    </div>
                    {output.error ? (
