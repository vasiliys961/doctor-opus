...
export default function Dicom3DViewer({ files, onClose, presentation = 'modal' }: Dicom3DViewerProps) {
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(() => {
      setError('MPR initialization timeout. Try a smaller series (first 80-100 slices) or reload the page.')
      setLoading(false)
    }, 25000)
  ...
  const initVtk = async () => {
    ...
    try {
      ...
      if (volumeRef.current) {
        try {
          ...
          setVolumeUnavailableMessage(
            volumeErr?.message
              ? `3D volume unavailable: ${volumeErr.message}`
              : '3D volume is unavailable in this browser. MPR slices continue to work.'
          );
        }
      }
      ...
      setError(err?.message || 'MPR initialization error');
      setLoading(false);
    }
  }
                  ...
                  {volumeUnavailableMessage && !loading && !error && (
                    <div className="absolute inset-0 z-20 flex items-center justify-center p-4 bg-black/70">
                      <div className="max-w-md text-center">
                        <p className="text-amber-300 text-sm font-semibold mb-2">3D volume rendering unavailable</p>
                        <p className="text-xs text-gray-300">{volumeUnavailableMessage}</p>
                        <p className="text-[10px] text-gray-400 mt-2">
                          Axial / Coronal / Sagittal continue to work normally.
                        </p>
                      </div>
                    </div>
                  )}
