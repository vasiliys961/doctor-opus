...
def show_consultation_protocol():
    ...
    if raw_text or st.session_state.get('raw_text'):
        ...
        if raw_text:
            ...
            # Инициализация таблицы промптов
            init_db()
            ...
            # Вкладки: выбор шаблона и управление промптами
            tab_template, tab_manage = st.tabs(["📝 Выбор шаблона", "⚙️ Управление промптами"])
            
            with tab_template:
                col_tpl1, col_tpl2 = st.columns(2)
                with col_tpl1:
                    # Получаем сохраненные промпты
                    ...
                    template_preset = st.selectbox(
                        "Выберите базовый шаблон или сохраненный промпт",
                        base_templates,
                        key="template_preset_select"
                    )
            ...
            with tab_manage:
                st.subheader("⚙️ Управление промптами специалистов")
                ...
                # Кнопка для генерации протокола (если еще не сгенерирован)
                if not st.session_state.get('structured_note'):
                    if st.button("🤖 Сгенерировать протокол", use_container_width=True, type="primary", key="generate_protocol_btn"):
                        with st.spinner("🤖 Генерация протокола..."):
                            assistant = OpenRouterAssistant()
                            
                            # Базовый промпт для терапевта (первый шаблон)
                            base_therapist_prompt = """Вы - опытный терапевт, американский профессор клинической медицины и ведущий специалист университетской клиники с многолетним клиническим опытом.
...
Вы совмещаете клиническую строгость и ответственность, давая ответы по клиническим проблемам внутренних болезней, включая акушерство и гинекологию, хирургию, а также помогаете обрабатывать несистемно изложенную информацию, облекая её по шаблону и стандартному протоколу осмотра терапевта с рекомендациями по обследованию и лечению."""
                            ...
                            # Второй базовый шаблон промпта (универсальный)
                            base_universal_prompt = """Вы - опытный врач-специалист, американский профессор клинической медицины и ведущий специалист университетской клиники с многолетним клиническим опытом.

Вы совмещаете клиническую строгость и ответственность, давая ответы по клиническим проблемам различных медицинских специальностей, а также помогаете обрабатывать несистемно изложенную информацию, облекая её по шаблону и стандартному протоколу осмотра с рекомендациями по обследованию и лечению."""
                            
                            # Выбираем базовый промпт в зависимости от выбранного шаблона
                            if template_preset == "Терапевт (по умолчанию)" or (selected_saved_prompt and selected_saved_prompt['specialist_name'].lower() == 'терапевт'):
                                base_prompt = base_therapist_prompt
                            else:
                                base_prompt = base_universal_prompt
...
"""
                        ...
                        st.rerun()  # Перезагружаем для показа результата
            ...
            if structured_note:
                ...
                with st.spinner("📄 Создание документа..."):
                    ...
                    with open(filepath, "rb") as f:
                        st.download_button(
                            ...
                            key="download_protocol_docx"
                        )
        
                # Кнопка для сохранения в контекст (если не сохранилось автоматически)
                if st.button("💾 Сохранить протокол в контекст пациента", key="save_protocol_context"):
                    try:
                        context_store = ContextStore()
                        context_store.add_context(
                            patient_id=patient_id,
                ...
                # Кнопка для очистки и начала заново
                if st.button("🔄 Создать новый протокол", key="reset_protocol"):
                    st.session_state.structured_note = ''
