...
def show_consultation_protocol():
    ...
    if audio and st.button("🎤 Обработать аудио"):
        ...
        # Блок выбора/загрузки шаблона протокола врача (для аудио-режима)
        st.subheader("🧩 Шаблон протокола врача (для расшифрованного аудио)")
        ...
        with col_tpl1:
            template_preset = st.selectbox(
                ...
                key="audio_template_preset",
            )
        with col_tpl2:
            uploaded_template = st.file_uploader(
                "Или загрузите свой шаблон (.txt/.md/.docx)",
                type=["txt", "md", "docx"],
                help="Текстовый файл или Word документ с вашим форматом протокола (разделы, формулировки, порядок блоков)",
                key="audio_template_file",
            )

        protocol_template = ""
        if uploaded_template is not None:
            try:
                file_extension = uploaded_template.name.split('.')[-1].lower() if uploaded_template.name else ""
                
                if file_extension == "docx":
                    if not DOCX_AVAILABLE:
                        st.error("❌ Для работы с .docx файлами требуется библиотека python-docx. Установите: pip install python-docx")
                        protocol_template = ""
                    else:
                        # Читаем .docx файл
                        ...
                        protocol_template = "\n".join(text_parts)
                        st.success("✅ Загружен пользовательский шаблон протокола врача (.docx)")
                else:
...
"""
    ...
    # --- Альтернативный режим: письменный ввод и загрузка готовых протоколов ---
    st.markdown("---")
    st.subheader("✏️ Текстовый ввод / загрузка готового протокола")

    # Источник текста: вручную или файл
    text_input_mode = st.radio(
        "Источник текста для протокола:",
        ["Ввести текст вручную", "Загрузить файл (.txt/.md/.docx/.pdf)"],
        horizontal=True,
        key="protocol_text_source",
    )

    manual_text = ""

    if text_input_mode == "Ввести текст вручную":
        manual_text = st.text_area(
            "Текст осмотра / черновик протокола",
            value="",
            height=200,
            key="manual_protocol_text",
            help="Сюда можно вставить уже написанный протокол или свободные заметки по приёму.",
        )
    else:
        uploaded_protocol = st.file_uploader(
            "Загрузите существующий протокол или черновик (.txt/.md/.docx/.pdf)",
            type=["txt", "md", "docx", "pdf"],
            key="protocol_source_file",
        )
        if uploaded_protocol is not None:
            try:
                ext = uploaded_protocol.name.split(".")[-1].lower() if uploaded_protocol.name else ""
                if ext in ["txt", "md"]:
                    manual_text = uploaded_protocol.read().decode("utf-8", errors="ignore")
                elif ext == "docx":
                    if not DOCX_AVAILABLE:
                        st.error("❌ Для работы с .docx файлами требуется библиотека python-docx. Установите: pip install python-docx")
                        manual_text = ""
                    else:
                        docx_bytes = uploaded_protocol.read()
                        docx_file = BytesIO(docx_bytes)
                        doc = Document(docx_file)
                    ...
                    try:
                        ...
                        pdf_bytes = uploaded_protocol.read()
                        ...
                        st.error(f"❌ Не удалось извлечь текст из PDF: {e}")
                        manual_text = ""
                if manual_text:
                    st.success("✅ Текст успешно загружен из файла")
            except Exception as e:
                st.error(f"❌ Ошибка чтения файла: {e}")
                manual_text = ""

    # Настройки промпта и образцов протоколов для специалиста
    with st.expander("🔧 Настройки промпта и образцов для специалиста", expanded=False):
        st.markdown(
            "Здесь можно загрузить **индивидуальный промпт** для конкретного специалиста "
            "и **2–3 примера протоколов**, чтобы модель копировала стиль и структуру."
        )

        custom_prompt_text = ""
        sample_protocols_text = ""

        prompt_file = st.file_uploader(
            "Загрузите промпт специалиста (.txt/.md/.docx)",
            type=["txt", "md", "docx"],
            key="protocol_prompt_file",
        )
        if prompt_file is not None:
            try:
                ext = prompt_file.name.split(".")[-1].lower() if prompt_file.name else ""
                if ext in ["txt", "md"]:
                    custom_prompt_text = prompt_file.read().decode("utf-8", errors="ignore")
                elif ext == "docx":
                    if DOCX_AVAILABLE:
                        docx_bytes = prompt_file.read()
                        docx_file = BytesIO(docx_bytes)
                        doc = Document(docx_file)
                        parts = [p.text for p in doc.paragraphs if p.text.strip()]
                        custom_prompt_text = "\n".join(parts)
                if custom_prompt_text:
                    st.success("✅ Промпт специалиста загружен")
            except Exception as e:
                st.warning(f"⚠️ Не удалось прочитать промпт: {e}")
                custom_prompt_text = ""

        sample_files = st.file_uploader(
            "Загрузите образцы протоколов (до 3 файлов, .txt/.md/.docx/.pdf)",
            type=["txt", "md", "docx", "pdf"],
            accept_multiple_files=True,
            key="protocol_samples_files",
        )

        samples_list = []
        if sample_files:
            for idx, sf in enumerate(sample_files[:3]):
                try:
                    ext = sf.name.split(".")[-1].lower() if sf.name else ""
                    text = ""
                    if ext in ["txt", "md"]:
                        text = sf.read().decode("utf-8", errors="ignore")
                    ...
                    if text and text.strip():
                        samples_list.append(text.strip())
                except Exception:
                    continue

        if custom_prompt_text or samples_list:
            st.info("✅ Индивидуальные настройки специалиста будут учтены при генерации протокола.")

    # Шаблон для текстового/файлового режима
    st.subheader("🧩 Шаблон протокола врача (для текстового режима)")
    col_mtpl1, col_mtpl2 = st.columns(2)
    with col_mtpl1:
        manual_template_preset = st.selectbox(
            "Выберите базовый шаблон",
            [
                "Терапевт (по умолчанию)",
                "Кардиолог",
            ...
            key="manual_template_preset",
        )
    with col_mtpl2:
        manual_uploaded_template = st.file_uploader(
            "Или загрузите свой шаблон (.txt/.md/.docx)",
            type=["txt", "md", "docx"],
            help="Текстовый файл или Word документ с вашим форматом протокола (разделы, формулировки, порядок блоков)",
            key="manual_template_file",
        )

    manual_protocol_template = ""
    if manual_uploaded_template is not None:
        try:
            ext = manual_uploaded_template.name.split(".")[-1].lower() if manual_uploaded_template.name else ""
            if ext in ["txt", "md"]:
                manual_protocol_template = manual_uploaded_template.read().decode("utf-8", errors="ignore")
            elif ext == "docx" and DOCX_AVAILABLE:
                docx_bytes = manual_uploaded_template.read()
                docx_file = BytesIO(docx_bytes)
                doc = Document(docx_file)
                parts = [p.text for p in doc.paragraphs if p.text.strip()]
                manual_protocol_template = "\n".join(parts)
            if manual_protocol_template:
                st.success("✅ Загружен пользовательский шаблон протокола врача")
        except Exception as e:
            st.warning(f"⚠️ Не удалось прочитать шаблон: {e}")
            manual_protocol_template = ""
        ...
        manual_protocol_template = preset_templates.get(manual_template_preset, "")

    manual_template_block = ""
    if manual_protocol_template and manual_protocol_template.strip():
        manual_template_block = (
            f"\nДополнительные требования к оформлению и содержанию протокола "
            f"в соответствии с шаблоном врача ({manual_template_preset}):\n{manual_protocol_template}\n"
        )

    # Кнопка генерации протокола из текста/файла
    if st.button("🤖 Сгенерировать протокол из текста/файла", type="primary"):
        if not manual_text or not manual_text.strip():
            st.error("❌ Нет текста для генерации протокола. Введите текст или загрузите файл.")
        else:
            with st.spinner("🤖 Генерация протокола..."):
                assistant = OpenRouterAssistant()

                extra_blocks = ""
                if manual_template_block:
                    extra_blocks += manual_template_block
                if custom_prompt_text:
                    extra_blocks += (
                        "\n=== ИНДИВИДУАЛЬНЫЙ ПРОМПТ СПЕЦИАЛИСТА ===\n"
                        + custom_prompt_text
                        + "\n"
                    )
                if samples_list:
                    extra_blocks += "\n=== ОБРАЗЦЫ ЖЕЛАЕМОГО ФОРМАТА ПРОТОКОЛА ===\n"
                    for i, sample in enumerate(samples_list, start=1):
                        extra_blocks += f"\n--- ОБРАЗЕЦ {i} ---\n{sample}\n"
...
Текст для обработки: {manual_text}
...
"""
            ...
            st.subheader("📄 Сгенерированный протокол (текст/файл)")
            st.write(structured_note)
            ...
            with st.spinner("📄 Создание документа..."):
                ...
                with open(filepath, "rb") as f:
                    st.download_button(
                        ...
                        file_name=os.path.basename(filepath),
                        mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                        key="download_manual_protocol",
                    )

            # Сохранение в клинический контекст
            if st.button("💾 Сохранить протокол (текст/файл) в контекст пациента"):
                try:
                    context_store = ContextStore()
                    context_store.add_context(
                        patient_id=patient_id,
                            ...
                            with st.spinner("🎤 Расшифровка аудио..."):
                                ...
                                if not api_key:
                                    ...
                                    if transcribed_text and not transcribed_text.startswith("❌"):
                                        # Извлекаем чистый текст из результата транскрипции
                                        # Убираем форматирование (🎙️, **, заголовки)
                                        clean_text = transcribed_text
                                        # Убираем префиксы типа "🎙️ **Расшифровка аудио**\n\n"
                                        if "**Расшифровка" in clean_text:
                                            # Находим начало текста после заголовка
                                            parts = clean_text.split("\n\n", 1)
                                            if len(parts) > 1:
                                                clean_text = parts[1]
                                        # Убираем разметку markdown
                                        clean_text = clean_text.replace("**", "").replace("🎙️", "").strip()
                                        
                                        # Сохраняем чистый текст в session_state
                                        st.session_state['transcribed_question'] = clean_text
                                        st.success(f"✅ Расшифровано: {clean_text[:100]}...")
