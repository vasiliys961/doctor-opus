...
def show_comparative_analysis_page():
    ...
    uploaded_files = st.file_uploader(
        ...
        accept_multiple_files=True,
        help="Оптимально 2-4 изображения"
    )
        ...
        try:
            ...
            for i, uploaded_file in enumerate(uploaded_files):
                try:
                    ...
                    result = analyzer.analyze_image(
                        image_array,
                        additional_context=f"Сравнительный анализ ({comparison_type}), изображение {i+1}"
                    )
            ...
            if results:
                ...
                st.subheader("🖼️ Анализируемые изображения")
                ...
                for i, (image, result) in enumerate(zip(images, results)):
                    with cols[i % len(cols)]:
                        ...
                        st.caption(f"Достоверность: {result.confidence:.1%}")
                ...
                if comparison_type == "Временная динамика":
                    ...
                    # Если есть количественные данные, строим графики
                ...
                with st.spinner("Генерация сравнительного заключения..."):
                    ...
                    comparison_prompt = f"""
Проведите сравнительный анализ {len(results)} медицинских изображений.
...
- Тип: {result.image_type.value}
- Достоверность: {result.confidence:.1%}
- Основные находки: {json.dumps(result.structured_findings, ensure_ascii=False, indent=2)}
"""
                    
                    comparison_prompt += """
Предоставьте:
1. Сравнительный анализ выявленных изменений
2. Динамику патологического процесса (если применимо)
3. Клинические рекомендации на основе сравнения
4. Прогностическую оценку

Ответ дайте в структурированном виде на русском языке.
"""
...
def integrate_with_main_app():
    ...
    # Добавить в список страниц основного приложения:
    enhanced_pages = [
    ...
    # В функции main() добавить обработку новых страниц:
    page_functions = {
